...
const PATH_IMAGES = "images/restaurants/";
...
function update(event) {
    ...
    const token = document.querySelector("#form_profile input[name='_token']").value;
    if (form != null)
        fetch("restaurants/update", { headers: { 'x-csrf-token': token }, method: "post", body: form }).then(onResponse, onError).then(onAccess);
}

function login() {
    const form = formLogin();
    const token = document.querySelector("#form_login input[name='_token']").value;
    if (form != null)
        fetch("restaurants/login", { headers: { 'x-csrf-token': token }, method: "post", body: form }).then(onResponse, onError).then(onAccess);
}

function register() {
    const form = formRegister();
    const token = document.querySelector("#form_register input[name='_token']").value;
    if (form != null)
        fetch("restaurants/register", { headers: { 'x-csrf-token': token }, method: "post", body: form }).then(onResponse, onError).then(onAccess);
}
...
function createBlocks() {
    ...
    for (const obj of myJson) {
        ...
        img_content.src = PATH_IMAGES + obj.image;
