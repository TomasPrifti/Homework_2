    ...
    <header>
        ...
        <div id="id_2">
            ...
            <div id="login">
                ...
                <div>
                    @if (!$existUserSup)
                        <input id= {{ $existUserRes ? "profile" : "input_login" }} , type="button", value= {{ $existUserRes ? "Profilo" : "Accedi" }} >
                        <span>oppure</span>
                        <input id= {{ $existUserRes ? "logout" : "input_register" }} , type="button" , value={{ $existUserRes ? "Logout" : "Registrati" }}>
                    @else
            ...
            <div id="modal_block", class="hidden">
                ...
                <div id="modal_content_profile" , class="hidden">
                    <form id="form_profile", enctype="multipart/form-data", method="post", action="{{ route('form_update') }}", class="fields">
                        @csrf
                        <label>Nome del Ristorante:<input id="profile_name" , type="text" , value="{{ $user != null ? $user->name : "" }}"></label>
                        <label>Indirizzo del Ristorante:<input id="profile_address" , type="text" , value="{{ $user != null ? $user->address : "" }}"></label>
                        <label id="description_label_profile">Descrizione del Ristorante:<textarea id="profile_description">{{ $user != null ? $user->description : "" }}</textarea></label>
